...
class StopNumAllWindowApply extends AllWindowFunction[Participant, StopNumber, TimeWindow] {
  ...
  def apply(window: TimeWindow, input: Iterable[Participant], out: Collector[StopNumber]): Unit = {
    ...
    for (par <- input) {
      ...
      //获取每个对象车 到 末端点D的距离
      val distance = (getDistance(longitude, latitude, lon, lat).toInt / 2).toInt
      arr.+=(distance)
    }
//    去重求出input的carid数量
...
//    取出占比大于百分之20 的数字
    ...
    val averStopNum = (intToInt.size/inputSize).toInt
    ...
    val stopNumber = StopNumber(eventStartStr, eventEndStr, averStopNum, 1)
